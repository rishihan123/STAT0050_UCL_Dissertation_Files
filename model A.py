...
def run_model_a_pagerank(season_year):
    ...
    for cat in categories:
        ...
        for game_id, game_group in cat_df.groupby("game_id"):
            ...
            if cat == "GK":
                ...
                game_group["metric_val"] = (
                    game_group["saves"] / total_saves if total_saves > 0 else 0
                )

            elif cat == "DF":
                # Defender score = weighted combination of tackles, interceptions, blocks and passes
                total_tkl = game_group["tackles_total"].sum()
                total_int = game_group["interceptions"].sum()
                total_blk = game_group["blocks"].sum()
                ...
                blk_share = game_group["blocks"] / total_blk if total_blk > 0 else 0
                ...
                # Midfielder score = weighted mix of total passes and key passes (chances created)
                total_passes = game_group["passes_total"].sum()
                ...
                # Forward score = weighted mix of goals, shots on target and assists
                total_goals = game_group["goals"].sum()
                total_sot = game_group["shots_on_target"].sum()
                total_ast = game_group["assists"].sum()

                goal_share = game_group["goals"] / total_goals if total_goals > 0 else 0
                sot_share = game_group["shots_on_target"] / total_sot if total_sot > 0 else 0
            ...
            game_group["metric_val"] = game_group["metric_val"] * (game_group["minutes_played"] / 90.0)
            ...
            for p1 in players:
                ...
                if target_strength_sum == 0:
                    ...
                    # Otherwise, stronger performers attract more of each sender's vote
                    for p2 in targets:
                        receiver = p2["player_name"]
        ...
        if len(G) > 0:
            ...
            res_df = res_df.merge(latest_team, left_on="Player", right_on="player_name", how="left")
...
if __name__ == "__main__":
    target_season = 2023
    results = run_model_a_pagerank(target_season)

    # Save CSV next to this script
    out_path = BASE_DIR / "modelA_pagerank_2023.csv"
    ...
    for cat in ["GK", "DF", "MF", "FW"]:
        print(f"\n--- Top 10 {cat}s ---")
        top = results[results["Category"] == cat].sort_values("Score", ascending=False).head(10)
